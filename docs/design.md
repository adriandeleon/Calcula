...
|---|---|---|---|
| **Trail** | JetBrains Mono 11 | 1 / 10 padding | Closeable with `C-x 1`, and its width is remembered. Sigil and text in separate boxes, so `=` and `!` keep a column *and* a wrapped line resumes under the text. Scrolls to the tail on every publish. 28 % of the split, on the right of the stack, not resizable with the parent. |
